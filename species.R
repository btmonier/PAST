...
server <- function(input, output) {
        ...
        if (input$type == "new")
            return(
                menuItem(
                    ...
                    # replace version.A, version.B, version.1, and version.2
                    # version.A/version.B should be replaced with the names of
                    # the GFFs without the extension
                    # example -> C.elegans.A.gff3 = C.elegans.A
                    selectInput("genes_file", "Annotations:", selected = NULL,
                                choices = c("Annotations" = "", "version.A", "version.B")),
                    
                    # version.1/version.2 should be replaced with the names of
                    # the files without the extension
                    # example -> C.elegans.1.txt = C.elegans.1
                    selectInput("pathway_file", "Pathways:", selected = NULL,
                                choices = c("Pathways" = "", "version.1", "version.2")),
                    
                    # Uncomment this line if you want to allow your users
                    # to select a number of cores. Otherwise, change the value
                    # on line <> to the desired number of cores.
    ...
    get_annotations = reactive({
        if (is.null(input$genes_file)) {
            return(NULL)
        } else if(input$genes_file == "") {
            return(NULL)
        } else {
            print(input$genes_file)
        }
        
        ## EDIT THIS BLOCK ##
        
        # change this extension to match your GFF files, whether you use
        # ".gff3" or ".gff"
        extension = ".gff3"
        
        # You can add more options here as long as you add them above.
        
        if (input$genes_file == "version.A") {
            return(paste0("path/to/gff/", "version.A", extension))
        } elif (input$genes_file == "version.B") {
            return(paste0("path/to/gff/", "version.B", extension))
    ...
    get_pathways_database = reactive({
        if (is.null(input$pathway_file)) {
            return(NULL)
        } else if(input$pathway_file == "") {
            return(NULL)
        } else {
            print(input$pathway_file)
        }
        
        ## EDIT THIS BLOCK ##
        
        # change this extension to match your pathway files
        extension = ".txt"
        
        # You can add more options here as long as you add them above.
        
        if (input$pathway_file == "version.1") {
            return(paste0("path/to/gff/", "version.1", extension))
        } elif (input$pathway_file == "version.2") {
            return(paste0("path/to/gff/", "version.2", extension))
    ...
    find_pathways <- reactive({
        genes <- assign_genes()
        pathway_file <- get_pathways_database()
